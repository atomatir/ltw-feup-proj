...
function searchPlaces($args) {
    ...
    $querySelectFrom = 'SELECT Place.name as name, Place.price_by_night as price, Place.placeID as id, Place.descrip as desc, Place.number_bedrooms as bed, Place.number_bathrooms as bath, Place.max_guests as guests, Place.price_by_night as price, Place.addressID as address
                 FROM Place';
    $queryWhere = ' WHERE Place.price_by_night >= :min_price AND
              Place.price_by_night <= :max_price';

    $queryArgs = array('min_price' => checkInput($args['min-price']), 'max_price' => checkInput($args['max-price']));

    // n_guests argument
    if (!empty($args['n-guests'])) {
        $queryWhere .= ' AND Place.max_guests >= :n_guests';
        $queryArgs['n_guests'] = checkInput($args['n-guests']);
    }
    ...
    if (!empty($args['cityID']) && $args['cityID'] != 'None') {
        ...
        $queryArgs['city'] = checkInput($args['cityID']); 
    } 
        ...
        $queryArgs['state'] = checkInput($args['states']);
    } 
        ...
        $queryArgs['country'] = checkInput($args['countries']);
    } 
        ...
        $queryArgs['region'] = checkInput($args['region']);
    }

    $query = $querySelectFrom . $queryWhere . " group by id";
